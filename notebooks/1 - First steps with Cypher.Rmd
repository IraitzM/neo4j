...
![](images/14.png)
Curious enough, Mrs. Andrenyi's husband, Count Rudolph Andrenyi also is traveling in the same train.
...
But count and countess Andrenyi travel on a separate compartments. He is in first class coach number 4.
...
So far we have played a little bit with Neo4J and we have seen some relevant aspects of it. For example, that no schema definition is enforced, at least by default.

The two main object types are nodes.

```{r}
"MATCH (n) RETURN *" %>% call_neo4j(con=con)
```

As can be seen, all nodes are part of the same type. Using LABEL we can set a logical grouping as it would happen with different TABLES in RDBMS so similar schema are expected in those cases.
...
```
Thanks to those labels and types we can just attack the subgraph we are interested in targeting the minimum amount of information two wrangle. Information that doesn't need to be denormalized as every instance is already linked to the rest of the information related to it. Lets take for example every suspect that visited victim's coach half an hour before his death.
...
Given that nodes are attached to other nodes via relations, Neo4J doesn't allow us to remove a node living its relations hanging.
